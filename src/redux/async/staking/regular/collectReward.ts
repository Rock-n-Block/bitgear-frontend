...
export const collectReward = async ({
  ...
  const type = stakingActionTypes.REGULAR_COLLECT_REWARD;
  try {
     ...
     */
    await contract.methods.withdraw(config.staking.regularPid, 0).send({
      from: userWalletAddress,
    });

    store.dispatch(apiActions.success(type));
  } catch (err) {
    console.log('Redux/Staking/Regular/collectReward', err);
